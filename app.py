...
# =========================================================
# Parsing 1 file Excel — otomatis deteksi format:
#   (A) 1 file = 1 matkul (format lama, sheet Info key-value)
#   (B) 1 file = banyak matkul (kolom "Mata Kuliah" di tiap sheet)
# Selalu mengembalikan dict: {course_key: {"error":.., "df":.., "weights_df":.., "meta":..}}
# =========================================================
def parse_workbook(uploaded_file):
    try:
        sheets = pd.read_excel(uploaded_file, sheet_name=None)
    except Exception as e:
        return {uploaded_file.name: {"error": f"Gagal membaca file: {e}"}}

    nilai_key = find_sheet(sheets, NILAI_SHEET_NAMES)
    if nilai_key is None:
        nilai_key = list(sheets.keys())[0]
    df_nilai_raw = normalize_columns(sheets[nilai_key].copy())
    df_nilai_raw = df_nilai_raw.rename(columns=MATKUL_COL_ALIASES)

    is_multi_course = "Mata Kuliah" in df_nilai_raw.columns

    if is_multi_course:
        return parse_multi_course_workbook(uploaded_file.name, sheets, nilai_key, df_nilai_raw)
    else:
        result = parse_single_course_workbook(uploaded_file.name, sheets, nilai_key, df_nilai_raw)
        return {uploaded_file.name: result}


def parse_multi_course_workbook(fname, sheets, nilai_key, df_nilai_raw):
    missing_nilai = [c for c in REQUIRED_COLS if c not in df_nilai_raw.columns]
    if missing_nilai:
        err = f"Sheet nilai ('{nilai_key}'): kolom hilang {missing_nilai}. Kolom tersedia: {list(df_nilai_raw.columns)}"
        return {f"{fname}": {"error": err}}

    bobot_key = find_sheet(sheets, BOBOT_SHEET_NAMES)
    if bobot_key is None:
        return {f"{fname}": {"error": "Sheet 'Bobot_CPL' tidak ditemukan (format multi-matkul butuh sheet ini)."}}

    df_bobot_raw = sheets[bobot_key].copy()
    df_bobot_raw = df_bobot_raw.rename(columns=lambda c: str(c).strip())
    df_bobot_raw = df_bobot_raw.rename(columns=MATKUL_COL_ALIASES)
    df_bobot_raw = df_bobot_raw.rename(columns=COLUMN_ALIASES)

    if "Mata Kuliah" not in df_bobot_raw.columns:
        return {f"{fname}": {"error": "Sheet Bobot_CPL tidak punya kolom 'Mata Kuliah' — "
                                       "wajib ada untuk format 1 file banyak matkul."}}

    # cari kolom kode CPL (kolom pertama setelah Mata Kuliah yang bukan komponen nilai)
    cpl_col_candidates = [c for c in df_bobot_raw.columns if c not in (["Mata Kuliah"] + components)]
    cpl_col = cpl_col_candidates[0] if cpl_col_candidates else None
    if cpl_col is None:
        return {f"{fname}": {"error": "Sheet Bobot_CPL: tidak ditemukan kolom kode CPL."}}
    df_bobot_raw = df_bobot_raw.rename(columns={cpl_col: "CPL"})

    missing_comp = [c for c in components if c not in df_bobot_raw.columns]
    if missing_comp:
        return {f"{fname}": {"error": f"Sheet Bobot_CPL: kolom komponen hilang {missing_comp}."}}

    df_bobot_raw["Mata Kuliah"] = df_bobot_raw["Mata Kuliah"].astype(str).str.strip()
    df_bobot_raw["CPL"] = df_bobot_raw["CPL"].astype(str).str.strip().str.upper().str.replace(" ", "")

    info_key = find_sheet(sheets, INFO_SHEET_NAMES)
    df_info_wide = None
    if info_key is not None:
        df_info_wide = sheets[info_key].copy()
        df_info_wide = df_info_wide.rename(columns=lambda c: str(c).strip())
        df_info_wide = df_info_wide.rename(columns=MATKUL_COL_ALIASES)

    df_nilai_raw["Mata Kuliah"] = df_nilai_raw["Mata Kuliah"].astype(str).str.strip()

    results = {}
    matkul_list = df_nilai_raw["Mata Kuliah"].dropna().unique()

    for matkul in matkul_list:
        course_key = f"{fname} — {matkul}"

        df_course = df_nilai_raw[df_nilai_raw["Mata Kuliah"] == matkul].drop(columns=["Mata Kuliah"]).reset_index(drop=True)
        if df_course.empty:
            continue

        df_bobot_course = df_bobot_raw[df_bobot_raw["Mata Kuliah"] == matkul]
        if df_bobot_course.empty:
            results[course_key] = {"error": f"Tidak ada baris Bobot_CPL untuk matkul '{matkul}'."}
            continue

        weights_df = df_bobot_course.set_index("CPL")[components].fillna(0.0).astype(float)
        weights_df = weights_df[weights_df.sum(axis=1) > 0]
        if weights_df.empty:
            results[course_key] = {"error": f"Bobot_CPL untuk '{matkul}' semuanya kosong/0."}
            continue

        meta = {"nama_matkul": matkul, "sks": 3, "kelas": "A", "dosen": "-"}
        if df_info_wide is not None and "Mata Kuliah" in df_info_wide.columns:
            row = df_info_wide[df_info_wide["Mata Kuliah"].astype(str).str.strip() == matkul]
            if not row.empty:
                row = row.iloc[0]
                if "SKS" in df_info_wide.columns and pd.notna(row.get("SKS")):
                    try:
                        meta["sks"] = int(float(row["SKS"]))
                    except Exception:
                        pass
                if "Kelas" in df_info_wide.columns and pd.notna(row.get("Kelas")):
                    meta["kelas"] = str(row["Kelas"])
                if "Dosen" in df_info_wide.columns and pd.notna(row.get("Dosen")):
                    meta["dosen"] = str(row["Dosen"])

        results[course_key] = {"error": None, "df": df_course, "weights_df": weights_df, "meta": meta}

    if not results:
        return {f"{fname}": {"error": "Tidak ada matkul valid ditemukan di file ini."}}

    return results


def parse_single_course_workbook(fname, sheets, nilai_key, df_nilai):
    """Format lama: 1 file = 1 matkul, sheet Info berbentuk key-value (Field/Value)."""
    ...
    # --- Sheet Info (opsional) ---
    meta = {
        "nama_matkul": os.path.splitext(fname)[0].replace("_", " "),
        "sks": 3,
        "kelas": "A",
        "dosen": "-",
    }
...
def generate_template_excel_multi_matkul():
    """Template format B: 1 file berisi banyak matkul, dibedakan kolom 'Mata Kuliah'."""
    df_nilai = pd.DataFrame({
        "Mata Kuliah": ["Algoritma"] * 3 + ["Basis Data"] * 3,
        "Nama": ["MHS_1", "MHS_2", "MHS_3"] * 2,
        "Tugas": [80, 85, 75, 78, 88, 70],
        "Partisipasi": [75, 80, 70, 74, 82, 68],
        "Proyek": [82, 88, 79, 80, 90, 72],
        "UTS": [78, 84, 72, 76, 86, 70],
        "Quiz": [77, 83, 74, 75, 85, 69],
        "UAS": [81, 87, 76, 79, 89, 73],
    })

    df_bobot = pd.DataFrame({
        "Mata Kuliah": ["Algoritma", "Algoritma", "Basis Data", "Basis Data"],
        "CPL": ["CPL1", "CPL2", "CPL2", "CPL3"],
        "Tugas": [20, 15, 15, 10],
        "Partisipasi": [10, 10, 10, 15],
        "Proyek": [20, 25, 25, 30],
        "UTS": [20, 20, 20, 20],
        "Quiz": [10, 10, 10, 10],
        "UAS": [20, 20, 20, 15],
...
with st.expander("📥 Download Template Excel", expanded=False):
    tcol1, tcol2 = st.columns(2)
    with tcol1:
        st.markdown("**Format A — 1 file = 1 matkul**")
        st.download_button(
            "Download Template (1 Matkul)",
            generate_template_excel(),
            file_name="Template_CPL_1Matkul.xlsx",
            use_container_width=True
        )
        st.caption(
            "Sheet **Nilai**: `Nama, Tugas, Partisipasi, Proyek, UTS, Quiz, UAS`  \n"
            "Sheet **Bobot_CPL**: baris=CPL, kolom=komponen (total per baris = 100)  \n"
            "Sheet **Info**: dua kolom `Field / Value`"
        )
    with tcol2:
        st.markdown("**Format B — semua matkul dalam 1 file**")
        st.download_button(
            "Download Template (Banyak Matkul)",
            generate_template_excel_multi_matkul(),
            file_name="Template_CPL_MultiMatkul.xlsx",
            use_container_width=True
        )
        st.caption(
            "Sama seperti Format A, tapi tiap sheet punya kolom tambahan **'Mata Kuliah'** "
            "untuk memisahkan data antar matkul. Sheet Info jadi tabel lebar: "
            "`Mata Kuliah | SKS | Kelas | Dosen` (1 baris per matkul)."
        )
    st.info("💡 Aplikasi otomatis mendeteksi format mana yang kamu upload — boleh dicampur keduanya sekaligus.")
...
st.caption(
    "Mendukung 2 format sekaligus — bisa dicampur dalam satu upload: "
    "**(A)** 1 file = 1 matkul (sheet Info berbentuk Field/Value), atau "
    "**(B)** 1 file = banyak matkul sekaligus (ada kolom **Mata Kuliah** di tiap sheet)."
)

uploaded_files = st.file_uploader(
    "Upload file Excel (boleh 1 file berisi banyak matkul, atau banyak file 1 matkul masing-masing)",
    type=["xlsx"],
...
if uploaded_files:
    ...
    for f in uploaded_files:
        parsed = parse_workbook(f)  # bisa menghasilkan >1 matkul dari 1 file
        st.session_state.courses_raw.update(parsed)
else:
